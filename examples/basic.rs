...
fn main() {
    ...
    history.push(Op::add_ok(1, 0u64, 1).at(Duration::from_millis(5)));

    // Process 1 adds element 2
    history.push(Op::add_invoke(2, 1u64, 2).at(Duration::from_millis(2)));
    history.push(Op::add_ok(3, 1u64, 2).at(Duration::from_millis(7)));

    // Process 2 reads and sees both elements
    history.push(Op::read_invoke(4, 2u64).at(Duration::from_millis(10)));
    history.push(Op::read_ok(5, 2u64, [1, 2]).at(Duration::from_millis(12)));
    ...
    // Add element 1
    bad_history.push(Op::add_invoke(0, 0u64, 1).at(Duration::from_millis(0)));
    bad_history.push(Op::add_ok(1, 0u64, 1).at(Duration::from_millis(5)));

    // First read sees it
    bad_history.push(Op::read_invoke(2, 1u64).at(Duration::from_millis(10)));
    bad_history.push(Op::read_ok(3, 1u64, [1]).at(Duration::from_millis(12)));

    // Second read doesn't see it - element was lost!
    bad_history.push(Op::read_invoke(4, 1u64).at(Duration::from_millis(20)));
    bad_history.push(Op::read_ok(5, 1u64, []).at(Duration::from_millis(22)));
