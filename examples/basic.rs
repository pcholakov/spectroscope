...
fn main() {
    ...
    history.push(Op::add_invoke(2, Pid(1), 2).at_millis(2));
    history.push(Op::add_ok(3, Pid(1), 2).at_millis(7));

    // Process 2 reads and sees both elements
    history.push(Op::read_invoke(4, Pid(2)).at_millis(10));
    history.push(Op::read_ok(5, Pid(2), [1, 2]).at_millis(12));
    ...
    // Add element 1
    bad_history.push(Op::add_invoke(0, Pid(0), 1).at_millis(0));
    bad_history.push(Op::add_ok(1, Pid(0), 1).at_millis(5));

    // First read sees it
    bad_history.push(Op::read_invoke(2, Pid(1)).at_millis(10));
    bad_history.push(Op::read_ok(3, Pid(1), [1]).at_millis(12));
